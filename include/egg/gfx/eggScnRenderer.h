...
public:
    ...
    enum ScnRendererFlag { RENDERER_VISIBLE = (1 << 0) };
...
public:
    ...
    bool isVisible() const {
        return mFlags & RENDERER_VISIBLE;
    }
...
#line 193
        ...
        if (localPrioOut != NULL)
            *localPrioOut = mpTimingPrioritySet[timing].localPrio;
        if (prioMaxOut != NULL)
            *prioMaxOut = mpTimingPrioritySet[timing].prioMax;
